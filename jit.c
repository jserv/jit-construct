...
void jit(const char *const file_contents)
{
	...
	GUARD(vector_push(&insn, prologue, sizeof(prologue)));
	...
	for (unsigned long i = 0; file_contents[i] != '\0'; ++i) {
		...
		case '>':
			// see: http://stackoverflow.com/a/8550253/1027966
			{
				...
				GUARD(vector_push(&insn, opcodes, sizeof(opcodes)));
			}
			break;
		case '<': {
				char opcodes [] = {
		...
		case '[': {
			...
			GUARD(stack_push(&relocation_table, insn.size)); // create a label after
			break;
		case ']': {
				char opcodes [] = {
					0x41, 0x80, 0x3C, 0x24, 0x00, // cmpb $0, (%r12)
			...
			relative_offset = insn.size - relocation_site;
			vector_write32LE(&insn, insn.size - 4, -relative_offset);
			vector_write32LE(&insn, relocation_site - 4, relative_offset);
			break;
		}
	}
	...
	GUARD(vector_push(&insn, epilogue, sizeof(epilogue)));

	void *mem = mmap(NULL, insn.size, PROT_WRITE | PROT_EXEC,
	                 MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	memcpy(mem, insn.data, insn.size);
	void (*jitted_func)(fn_memset, fn_putchar, fn_getchar) = mem;
	jitted_func(memset, putchar, getchar);
	munmap(mem, insn.size);
	vector_destroy(&insn);
}
